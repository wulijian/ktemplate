 ...
 * 编译模版代码
 * @param code html模版代码
 * @param helperName html模版帮助代码
 * @param isEs6 是否使用es6模式
 * @return {Function}  模版函数，接收一个数据，返回html代码
 */
var compile = function (code, helperName, isEs6) {
    isEs6 = !!isEs6; //默认false
    var tempCode = '';
    var fileName = path.basename(fileRealPath);  //文件名
    if (!!isEs6) {
        tempCode += '"es6insertposition";\n';
    }
    ...
    code.split(SYNTAXSIGN.start).forEach(function (codeSnip) {
        ...
        var htmlCode = codeArr[0];
        var jsCode = codeArr[1];
        if (codeArr.length === 1 && htmlCode !== '') {
            tempCode += html(htmlCode, fileName);
        } else {
            tempCode += js(htmlCode, fileName, helperName);
            if (typeof jsCode !== 'undefined' && jsCode !== '') {
                tempCode += html(jsCode, fileName);
            }
        }
    });
    if (!isEs6) {
 ...
 * @param helperName html模版帮助js方法
 * @return {string} 处理后的js代码
 */
var js = function (code, fileName, helperName) {
    ...
    if (code.indexOf(SYNTAXSIGN.dval) === 0) {
        handledCode = replaces[1] + (helperName || moduleName) + '.escape(' + code.substring(1).replace(/[\s;]*$/, '') + ')' + replaces[2];
    }
    return handledCode + '\n';
};
 ...
 * @param helperName 帮助模块名称
 * @param isEs6 是否es6模式
 * @return {Function}  模版代码生成的js函数
 */
exports.compile = function (templatePath, helperName, isEs6) {
    resetAll();
    ...
    return compile(subToCombinedTemplateNode.toString(), helperName, isEs6);
};
/**
 * 生成sourceMap
